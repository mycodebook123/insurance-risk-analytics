...
def run_provinces(df):
    ...
    return {
        ...
        'Test': 'Chi-Squared', 'KPI': 'Claim Frequency',
        'p_value': round(p, 6), 'Decision': decision, 'Interpretation': interpretation
    }


def run_zipcodes_risk(df):
    ...
    if len(top_zips) < 2:
        return {'Hypothesis': 'H2', 'Test': 'T-Test', 'KPI': 'Claim Severity',
                'p_value': 1.0, 'Decision': 'Fail to Reject H0', 'Interpretation': 'Not enough zip codes'}
    ...
    interpretation = (
        ...
        if p < 0.05 else
        "No significant claim severity difference between the two largest zip codes."
    )
    return {
        'Hypothesis': 'H2: No risk diff between zip codes',
        'Test': 'Welch T-Test', 'KPI': 'Claim Severity',
        'p_value': round(p, 6), 'Decision': decision, 'Interpretation': interpretation
    }


def run_zipcodes_margin(df):
    """H3: No margin difference between zip codes. Uses t-test on Margin."""
    top_zips = df['PostalCode'].value_counts().head(2).index.tolist()
    if len(top_zips) < 2:
        return {'Hypothesis': 'H3', 'Test': 'T-Test', 'KPI': 'Margin',
                'p_value': 1.0, 'Decision': 'Fail to Reject H0', 'Interpretation': 'Not enough zip codes'}
...
def run_gender(df):
    ...
    if contingency.shape != (2, 2):
        return {'Hypothesis': 'H4', 'Test': 'Chi-Squared', 'KPI': 'Claim Frequency',
                'p_value': 1.0, 'Decision': 'Fail to Reject H0', 'Interpretation': 'Gender data insufficient'}
    chi2, p, dof, _ = stats.chi2_contingency(contingency)
    decision = 'Reject H0' if p < 0.05 else 'Fail to Reject H0'
    interpretation = (
